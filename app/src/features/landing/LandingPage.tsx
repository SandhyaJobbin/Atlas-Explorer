...
const STEPS = [
  ...
  { num: '02', label: 'Geo Tiles',     desc: 'Touch every tile to reveal timezone and coast info',   tag: 'Train' },
  { num: '03', label: 'Code Drop',     desc: 'Catch the right state code before the block lands',   tag: 'Play'  },
  { num: '04', label: 'Pin Rush',      desc: 'Tap the right target while the map is live',          tag: 'Play'  },
  { num: '05', label: 'City Stack',    desc: 'Drag city cards into the right zone for combo points', tag: 'Play'  },
];
...
export default function LandingPage() {
  const { createNewSession, createDemoSession, session } = useSession();
  ...
  function handleResume() {
    ...
    if (session.completed) {
      ...
      navigate('/play');
    } else if (session.training.mapExplorerClicked.length >= 63) {
      navigate('/train/tiles');
    } else {
      navigate('/train/map');
    }
  }
  ...
  return (
    <AppLayout>
      <main className="flex flex-col lg:flex-row h-screen w-screen overflow-hidden bg-[#F5F0E8] relative">
        ...
        <section className="relative z-10 w-full lg:w-[450px] flex-shrink-0 flex flex-col p-6 lg:p-12 overflow-y-auto lg:overflow-visible border-r border-[#2D3B2F]/5 bg-[#F5F0E8]/80 backdrop-blur-sm">
          <div className="mb-10">
            ...
            <p className="text-[#2D3B2F]/60 text-sm max-w-xs">
              Welcome, Agent. Complete the training modules to unlock the tactical simulations.
            </p>
          </div>
        ...
        <section className="hidden lg:flex flex-1 flex-col p-12 bg-[#232F3E] relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-12">
              <div className="space-y-4">
                ...
                <div className="flex gap-8">
                  {[
                    { val: '03', label: 'Playable Levels' },
                    { val: '70%', label: 'Min Accuracy' },
                    { val: '09', label: 'Max Stars' },
                  ].map(item => (
                    <div key={item.label}>
            ...
            <div className="flex-1 flex flex-col justify-center gap-12">
              <div className="max-w-md">
                ...
                <p className="text-white/50 text-lg leading-relaxed">
                  The ultimate geographic precision challenge. Master the North America sector through three high-stakes simulations.
                </p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                {STEPS.slice(2).map((step, i) => (
                  <AnimatedCard key={step.num} tiltAmount={4} className="p-6 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-md">
                    <div className="w-10 h-10 rounded-xl bg-[#FF9900]/20 flex items-center justify-center text-[#FF9900] font-black text-sm mb-4">
                      {step.num}
                    </div>
                    <h3 className="text-white font-black text-lg mb-2">{step.label}</h3>
